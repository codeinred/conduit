...
class callback {
   ...
   public:
    ...
    callback(callback&& source) noexcept
      : h(release(source.h)) {}
    // Releases ownership of the coroutine and produces an async::jump
    // with that coroutine
    [[nodiscard]] inline auto release() noexcept -> jump {
        return jump {release(h)};
    }
    inline void emplace(std::coroutine_handle<> handle) noexcept { h = handle; }
    ...
    inline void resume() { release(h).resume(); }
    inline void operator()() { release(h).resume(); }
    ~callback() {
        if (h)
            h.destroy();
    }
